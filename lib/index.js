...
var scigen = function scigen(authors, bibInLatex) {
  ...
  var prettyPrint = function prettyPrint(text) {
    text = text.split('\n').map(function (line) {
      ...
      if (title) {
        ...
        line = line.replace(/((jan)|(feb)|(mar)|(apr)|(may)|(jun)|(jul)|(aug)|(sep)|(oct)|(nov)|(dec))\s/gi, function (l) {
          return l.substring(0, l.length) + '. ';
        });
      }
  ...
  return {
    files: _objectSpread({
      'paper.tex': bibInLatex ? text.replace(/\\cite\{((cite:\d+(, )?)+)\}/g, function () {
        for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
          args[_key] = arguments[_key];
        }

        var a = '[' + args[1].split(', ').map(function (c) {
          return c.replace(/cite:/, '');
        }).join(', ') + ']';
        return a;
      }).replace(/\\bibliography\{scigenbibfile\}/, "\\section*{References}\n\n             \\begin{enumerate}[label={[\\arabic*]}, labelindent=0pt]\n" + bibtex(rules, true).replace(/\\textsc\{(.*)\}\. /g, function (match, authors) {
